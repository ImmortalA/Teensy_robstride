...
int main()
{
    ...
    // -------- MIT-only demo (no mode selection) --------

    // Force O2 RUN_MODE (0x7005) = 0 (operation) so motor accepts Type 1 in the loop.
    ...
    // Always use MIT packing in this demo.
    for (auto &board : _spine_boards)
    {
        board->setAllowCommandSend(true);
        board->setUseMitPack(true);
    }

    std::cout << "Running MIT sine control demo (Ctrl+C to stop)...\n";

    // Setpoints for MIT mode (rad, rad/s, Nm). Larger amplitude = more visible motion.
    const float MIT_SINE_AMP = 1.0f;       // MIT sine amplitude (rad) — 1.0 ≈ 57°
    ...
    // Center sine around current position so we don't command one big step.
    float p_center = 0.0f;
    ...
    while (true)
    {
        ...
        for (size_t i = 0; i < ACTUATOR_INFO_MAP.size(); i++)
        {
            ...
            // MIT sine: position oscillates around p_center, velocity is derivative of sine
            float t = iter * dt;
